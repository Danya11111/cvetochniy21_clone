...
|-------|------|---------|------------|
| orders | total | рубли | При оформлении до оплаты = сумма к оплате после бонусов (как на витрине) |
| orders | total_paid | копейки | **Только фактически полученная оплата** (после T-Bank `CONFIRMED`); до оплаты = `0` |
